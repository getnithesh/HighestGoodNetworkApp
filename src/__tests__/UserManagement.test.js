...
describe("User Management Component", () => {
  ...
  describe("Behavior", () => {
    it("verifying the active filter", () => {
      const event = {
        target: { name: "active-filter-dropdown", value: "active" }
      };
    ...
    it("verifying the wild card search", () => {
      const event = {
        target: { name: "user-profiles-wild-card-search", value: "Admin" }
      };
      ...
      userManagementComponent
        .find("#user-profiles-wild-card-search")
        .simulate("change", event);
    ...
    it("verifying the column filters", () => {
      const event = {
        target: {
          name: "search_email_search",
          value: "onecommunityglobal@gmail.com"
        }
      };
      ...
      const dropdownevent = {
        target: { name: "search_role_search", value: "Volunteer" }
      };
      userManagementComponent
        .find("#search_role_search")
        .simulate("change", dropdownevent);
      // verifying  that there is only 1 row after choosing the role from the drop down.
      expect(userManagementComponent.find(".usermanagement__tr").length).toBe(
        1
      );

      const commitedhrsevent = {
        target: { name: "search_hrs_search", value: "10" }
      };
      userManagementComponent
        .find("#search_hrs_search")
        .simulate("change", dropdownevent);
